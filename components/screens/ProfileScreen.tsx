...
export default function ProfileScreen() {
  ...
  return <AppShell title="Profile"><main className="screen profile-screen settings-screen workspace-destination">
    ...
    <section className="account-card panel quiet-card"><span className="account-avatar">{user.avatarUrl && !avatarFailed ? <img src={user.avatarUrl} alt="" onError={() => setAvatarFailed(true)} /> : initial}</span><span className="account-copy"><strong>{displayName}</strong><small>{user.login ? `@${user.login}` : "Personal workspace"}</small></span><ChevronRight size={17} className="account-arrow" /></section>
    <section className="profile-group"><div className="profile-group-label">Workspace</div><div className="profile-card panel quiet-card"><Row icon={<Sparkles size={17} />} label="Approvals" href="/approvals" /><Row icon={<Database size={17} />} label="Memory" href="/memory" /><Row icon={<Sparkles size={17} />} label="Improve Elias" href="/improvements" /><Row icon={<Sparkles size={17} />} label="Skills" detail="Enable capabilities and review contributors" href="/skills" /><Row icon={<Sparkles size={17} />} label="Search" href="/search" /></div></section>
    <section className="profile-group"><div className="profile-group-label">Connections</div><div className="profile-card panel quiet-card"><Row icon={<Puzzle size={17} />} label="Connectors" detail={connected} href="/connectors" /><Row icon={<ShieldCheck size={17} />} label="Privacy" href="/privacy" /></div></section>
    <section className="profile-group"><div className="profile-group-label">Account</div><div className="profile-card panel quiet-card"><Row icon={<UserCircle size={17} />} label="Account" detail={user.email || user.login || "Personal"} /><Row icon={<Moon size={17} />} label="Appearance" detail="System" /></div></section>
    <section className="profile-group profile-actions"><div className="profile-card panel quiet-card"><Row icon={<CircleHelp size={17} />} label="Help" /><Row icon={<LogOut size={17} />} label="Sign out" danger onClick={() => void logout()} /></div></section>
  </main></AppShell>;
}
