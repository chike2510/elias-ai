...
export default function ProfileScreen() {
  ...
  return <AppShell title="Profile"><main className="screen profile-screen"><div className="profile-titlebar"><Link href="/chat" aria-label="Back to chat"><ArrowLeft size={19} /></Link><span className="profile-wordmark">Profile</span><span /></div><section className="account-card"><span className="account-avatar">{user.avatarUrl ? <img src={user.avatarUrl} alt="" /> : initial}</span><span><strong>{user.name || user.login || "Your account"}</strong><small>{user.login ? `@${user.login}` : "Personal workspace"}</small></span></section><section className="profile-group"><div className="profile-group-label">Workspace</div><Row icon={<Sparkles size={18} />} label="Approvals" href="/approvals" /><Row icon={<Database size={18} />} label="Memory" href="/memory" /><Row icon={<Sparkles size={18} />} label="Improve Elias" href="/improvements" /><Row icon={<Sparkles size={18} />} label="Skills" detail="Enable capabilities and review contributors" href="/skills" /><Row icon={<Sparkles size={18} />} label="Search" href="/search" /></section><section className="profile-group"><div className="profile-group-label">Connections</div><Row icon={<Puzzle size={18} />} label="Connectors" detail={connected} href="/connectors" /><Row icon={<ShieldCheck size={18} />} label="Privacy" href="/privacy" /></section><section className="profile-group"><div className="profile-group-label">Account</div><Row icon={<UserCircle size={18} />} label="Account" detail={user.email || user.login || "Personal"} /><Row icon={<Moon size={18} />} label="Appearance" detail="System" /></section><section className="profile-group profile-actions"><Row icon={<CircleHelp size={18} />} label="Help" /><button className="profile-row danger-row" type="button" onClick={() => void logout()}><span className="profile-row-icon"><LogOut size={18} /></span><span className="profile-row-copy"><strong>Sign out</strong></span></button></section></main></AppShell>;
}
