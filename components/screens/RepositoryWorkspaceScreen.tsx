...
export default function RepositoryWorkspaceScreen({ owner, repo }: { owner: string; repo: string }) {
  ...
  async function runModelReview() {
    ...
    try {
      const response = await fetch(`/api/github/repositories/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/review`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paths: selectedPaths, diff: reviewDiff, branch: intelligence.branch }) });
      const payload = await response.json();
