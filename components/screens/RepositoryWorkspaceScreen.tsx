...
export default function RepositoryWorkspaceScreen({ owner, repo }: { owner: string; repo: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const payload: Record<string, unknown> = { action: writeAction, owner, repo };
      ...
      const proposalResponse = await fetch("/api/github/actions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...payload, phase: "prepare" }) });
      const proposal = await proposalResponse.json() as { proposalId?: string; expiresAt?: number; message?: string };
      if (!proposalResponse.ok || !proposal.proposalId) throw new Error(proposal.message || "Could not prepare the GitHub action.");
      const explanation = writeAction === "create_branch" ? `Create branch “${String(payload.branch)}” from “${String(payload.base)}”?` : writeAction === "commit_file" ? `Commit “${String(payload.path)}” to “${String(payload.branch)}”?` : writeAction === "create_pull_request" ? `Open a pull request from “${String(payload.head)}” into “${String(payload.base)}”?` : writeAction === "create_issue" ? `Create this issue in “${owner}/${repo}”?` : `Post this review comment to “${owner}/${repo}”?`;
      if (!window.confirm(`${explanation}\n\nThis is a real GitHub write action. The approval expires in ${Math.max(1, Math.ceil(((proposal.expiresAt || Date.now()) - Date.now()) / 60_000))} minutes and is bound to the exact submitted fields.`)) { setNotice("GitHub action not approved."); return; }
      const response = await fetch("/api/github/actions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...payload, phase: "execute", proposalId: proposal.proposalId, confirm: `CONFIRM_GITHUB_${writeAction.toUpperCase()}` }) });
