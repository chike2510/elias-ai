    ...
    <AppShell title="Tasks">
      <main className="screen task-workspace-screen">
        ...
        <section className="task-objective panel">
          <div className="task-objective-label"><ClipboardCheck size={15} /> What should Elias do?</div>
          <textarea value={objective} onChange={(event) => setObjective(event.target.value)} disabled={busy && Boolean(task)} rows={3} placeholder="Describe the outcome…" />
          <div className="task-objective-actions"><small>{task ? `${task.kind} · ${task.workspace.length} files` : ""}</small><button type="button" className="primary" disabled={!objective.trim() || busy} onClick={() => task ? void run() : void create()}>{busy ? <LoaderCircle size={15} className="spin" /> : task ? <Play size={15} /> : <Send size={15} />}{busy ? "Working" : task ? "Continue" : "Start task"}</button></div>
        </section>
        ...
        {task ? <>
          ...
          <div className="task-grid">
            <section className="task-plan panel"><div className="workbench-section-head"><div><span className="eyebrow">PLAN</span><h2>Steps</h2></div><ShieldCheck size={17} /></div><div className="plan-list">{task.plan.map((step, index) => <div className={`plan-step ${step.status}`} key={step.id}><span className="plan-index">{step.status === "completed" ? <Check size={13} /> : index + 1}</span><div><strong>{step.title}</strong><p>{step.description}</p>{step.evidenceEventIds.length ? <small>{step.evidenceEventIds.length} evidence event{step.evidenceEventIds.length === 1 ? "" : "s"}</small> : null}</div><span className="plan-state">{step.status}</span></div>)}</div></section>
            <section className="task-activity panel"><div className="workbench-section-head"><div><span className="eyebrow">ACTIVITY</span><h2>Activity</h2></div><span className="activity-count">{task.events.length}</span></div><div className="evidence-list">{task.events.length ? [...task.events].reverse().map((event) => <article className={`evidence-item ${event.status}`} key={event.id}><span className="evidence-line" /><div><div className="evidence-meta"><strong>{event.label}</strong><time>{time(event.createdAt)}</time></div>{event.detail ? <p>{event.detail}</p> : null}{event.evidence ? <pre>{typeof event.evidence.value === "string" ? event.evidence.value : JSON.stringify(event.evidence.value, null, 2)}</pre> : null}</div></article>) : <div className="task-empty"><Sparkles size={20} /><strong>No activity yet</strong><small>Start the task and Elias will show the actual operations and evidence here.</small></div>}</div></section>
          </div>
