...
export default function FilesScreen() {
  ...
  return <AppShell title="Library"><main className="screen library-screen workspace-destination"><header className="screen-header"><div className="screen-header-copy"><span className="eyebrow">FILES</span><h1>Library</h1><p className="screen-description">Files uploaded or created by Elias, ready to revisit.</p></div><span className="library-count quiet-badge">{filtered.length}</span></header><div className="searchbox library-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search files" /><span className="search-hint">{filtered.length}</span></div><div className="file-list library-list">{filtered.map((artifact) => <div className="file-row library-row" key={artifact.id}><span className={`file-icon file-type-${typeFor(artifact.name).toLowerCase()}`}>{iconFor(artifact.name)}<small>{typeFor(artifact.name)}</small></span><span className="file-row-main"><strong>{artifact.name}</strong><small>{artifact.pageCount ? `${artifact.pageCount} pages · ` : ""}{artifact.chunks?.length ? `${artifact.chunks.length} chunks · ` : ""}{new Date(artifact.createdAt).toLocaleDateString()}</small></span><span className="file-row-actions"><button type="button" className="secondary preview-button" onClick={() => artifact.text !== undefined && setPreview(preview === artifact.id ? null : artifact.id)}>{artifact.text !== undefined ? "Preview" : "View"}</button>{artifact.blob || artifact.text !== undefined ? <button type="button" className="icon-btn" title="Download" onClick={() => download(artifact)}><Download size={16} /></button> : null}<button type="button" className="icon-btn danger-icon" title="Delete" onClick={() => { void deleteArtifact(artifact.id).then(reload); }}><Trash2 size={16} /></button></span>{preview === artifact.id && artifact.text !== undefined ? <div className="artifact-preview"><strong>{artifact.summary ? "Summary" : "Text"}</strong><pre>{artifact.text}</pre>{artifact.chunks?.length ? <small>{artifact.chunks.length} searchable chunks</small> : null}</div> : null}</div>)}{!filtered.length ? <div className="empty-state panel"><Archive size={22} /><b>{query ? "No matches" : "No files yet"}</b><small>{query ? "Try another search." : "Files uploaded or created by Elias appear here."}</small><a className="primary" href="/chat">Open Chat <ArrowUpRight size={14} /></a></div> : null}</div></main></AppShell>;
}
