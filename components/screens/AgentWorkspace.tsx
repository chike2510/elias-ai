  ...
  return (
    <AppShell title={projectName}>
      <main className="screen agent-workspace">
        ...
        <section className="agent-source panel"><div className="agent-source-heading"><div><span className="eyebrow">workspace</span><h2><Folder size={17} /> {projectName}</h2><small><CheckCircle2 size={12} /> Local workspace ready</small></div><Link href="/projects" className="secondary"><Link2 size={14} /> connect</Link></div><div className="agent-source-actions"><button type="button" className="agent-action-primary" onClick={() => filesInput.current?.click()}><Paperclip size={15} /> Attach files</button><button type="button" onClick={() => zipInput.current?.click()}><Archive size={15} /> Import ZIP</button><button type="button" onClick={() => filesInput.current?.click()}><FolderPlus size={15} /> Add folder</button><button type="button" onClick={() => void downloadZip()}><Archive size={15} /> Export</button></div></section>
