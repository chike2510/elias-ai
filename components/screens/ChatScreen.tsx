      ...
      <main className="screen chat-screen">
        ...
        <div className="chat-composer">
          ...
          <div className="chat-composer-bar">
            <div className="composer-left"><div className="composer-plus-wrap"><button type="button" className="composer-plus" aria-label="Add attachment or connection" aria-expanded={plusOpen} onClick={() => setPlusOpen((open) => !open)}><Plus size={19} /></button>{plusOpen ? <div className="composer-plus-menu"><strong>Add to this conversation</strong><button type="button" onClick={() => { uploadRef.current?.click(); setPlusOpen(false); }}><Paperclip size={15} /> Attach file</button><a href="/api/connect/github"><Link2 size={15} /> Connect GitHub</a><Link href="/connectors/vercel"><FolderPlus size={15} /> Connect Vercel via MCP</Link><Link href="/projects"><FolderPlus size={15} /> Add project context</Link><button type="button" onClick={() => { setPlusOpen(false); setInput("Describe the task you want Elias to run"); }}><Sparkles size={15} /> Start a task in chat</button></div> : null}</div><Link href="/studio?mode=voice" className="composer-utility" title="Voice" aria-label="Voice"><Mic size={17} /></Link><Link href="/studio?mode=camera" className="composer-utility" title="Camera" aria-label="Camera"><Camera size={17} /></Link></div>
