    ...
    <AppShell title="Chat">
      <main className="screen chat-screen chat-route-screen">
        <div className="chat-body">
          {!messages.length ? <div className="chat-empty"><div className="brand-mark large"><Sparkles size={27} /></div><h2>What are we working on?</h2><p>Ask anything. Add context only when you need it.</p><div className="chat-suggestions"><button type="button" onClick={() => setInput("review this project architecture and tell me what you would improve")}><span>Review a project</span><ChevronRight size={14} /></button><button type="button" onClick={() => setInput("research the latest changes in Next.js and cite your sources")}><span>Research something current</span><ChevronRight size={14} /></button><button type="button" onClick={() => setInput("teach me this topic like an exam tutor")}><span>Study with ELIAS</span><ChevronRight size={14} /></button></div></div> : null}
