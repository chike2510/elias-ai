...
export async function GET(request: Request) {
  ...
  if (!githubConfigured(flow)) return NextResponse.redirect(new URL(`${flow === "connect" ? "/projects" : "/login"}?error=${flow === "connect" ? "github_repository_not_configured" : "github_login_not_configured"}`, request.url));
  ...
  if (!tokenResponse.ok || !tokenData.access_token) return NextResponse.redirect(new URL(`${flow === "connect" ? "/projects" : "/login"}?error=${encodeURIComponent(tokenData.error || "github_token_exchange")}`, request.url));
  ...
  if (!profileResponse.ok || !profile.id || !profile.login) return NextResponse.redirect(new URL(`${flow === "connect" ? "/projects" : "/login"}?error=github_profile`, request.url));
  ...
  if (flow === "connect" && previous && previous.userId !== `github_${profile.id}`) {
    return NextResponse.redirect(new URL("/projects?error=github_account_mismatch", request.url));
  }
  ...
  const { githubToken: _githubToken, githubConnected: _githubConnected, ...sessionWithoutRepository } = previous || {};
